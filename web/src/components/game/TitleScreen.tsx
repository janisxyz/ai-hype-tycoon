...
export function TitleScreen({
  ...
  return (
    <div className="relative min-h-dvh overflow-hidden bg-bg">
      <img src="/hq/title.jpg" alt="" className="absolute inset-0 h-full w-full object-cover" />
      ...
      <div className="relative z-10 mx-auto flex min-h-dvh max-w-xl flex-col justify-end px-5 pb-10 pt-16 sm:justify-center sm:pb-16">
        ...
        <p className="mt-5 max-w-md text-sm text-paper/80 sm:text-base">
          Raise, hire, train, ship. Go public. Keep going — the tape never ends.
        </p>
        ...
        <div className="mt-8 grid grid-cols-3 gap-2">
          {[
            ...
            <div key={item.k} className="rounded-lg border border-border bg-bg/55 px-3 py-3">
              <p className="font-mono text-[10px] tracking-[0.16em] text-subtle">{item.k}</p>
              <p className="mt-1 font-display text-lg italic leading-none text-paper">{item.v}</p>
            </div>
          ))}
        </div>
      </div>
