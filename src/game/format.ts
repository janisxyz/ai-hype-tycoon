...
export function yearOf(day: number): number {
  return 2024 + Math.floor(day / 365);
}

export function tokLabel(n: number): string {
  const a = Math.abs(n);
  if (a >= 1_000_000_000) return `${(a / 1_000_000_000).toFixed(1)}B tok/s`;
  if (a >= 1_000_000) return `${(a / 1_000_000).toFixed(1)}M tok/s`;
  if (a >= 1_000) return `${(a / 1_000).toFixed(1)}k tok/s`;
  return `${Math.round(a)} tok/s`;
}
