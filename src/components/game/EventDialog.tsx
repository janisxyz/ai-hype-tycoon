export function EventDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink/70 p-3 sm:items-center">
      <div className="w-full max-w-lg rounded-xl border border-border bg-surface p-5 shadow-soft">
        <p className="kicker text-warn">Decision</p>
        ...
        <div className="mt-5 flex flex-col gap-2">
          {choices.map((c) => (
            <button
              ...
              type="button"
              onClick={() => onPick(c.id)}
              className="rounded-md border border-border bg-elevated px-4 py-3 text-left transition-colors hover:border-border-strong"
            >
